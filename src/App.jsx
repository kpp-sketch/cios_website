...
export default function App() {
  ...
  const managementTeam = [
    ...
    { name: 'Eva Myšáková', role: 'Financial Manager', email: 'eva.mysakova@prf.cuni.cz', photo: 'eva_mysakova.png', bio: '' },
    { name: 'Kateřina Pospíchalová Pavlov', role: 'Administrator', email: 'katerina.pospichalovapavlov@prf.cuni.cz', photo: 'katerina_pavlov.png', bio: '' },
 ...
 const autoLinkNames = (text) => {
  ...
  {alphabet.map(letter => (
    presentLetters.has(letter) ? (
                ...
                {isabMembers.map((member, idx) => (
                  <div key={idx} className="flex flex-col sm:flex-row gap-8 items-start pb-8 border-b border-slate-100 last:border-0">
                    ...
                    <div className="flex-grow pt-1">
                      ...
                      <div className="flex flex-wrap gap-6 text-sm font-bold">
                        <a href={`mailto:${member.email}`} className="hover:underline flex items-center" style={{ color: colors.navy }}><Mail className="w-4 h-4 mr-2 opacity-70" /> {member.email}</a>
                        <a href={member.link} target="_blank" rel="noreferrer" className="hover:underline flex items-center" style={{ color: colors.navy }}><Globe className="w-4 h-4 mr-2 opacity-70" /> Personal Website</a>
                      </div>
                    </div>
                  </div>
                ))}
            ...
            <div id="work-packages" className="scroll-mt-32">
              ...
              <div className="grid gap-10">
                {[
                  ...
                  { id: 'WP2', title: 'Inequality in Justice', leader: 'Michal Šoltés', desc: 'This work package investigates a range of research questions related to inequalities in judicial decision-making and access to justice, drawing on unique data collected from criminal justice system stakeholders and inmates. The topics examined include the effects of judges’ personality traits and perceptual biases on sentencing decisions, discriminatory behavior by lawyers toward vulnerable individuals, and the broader societal consequences of judicial disparities. The findings are expected to advance knowledge at the intersection of law and economics and to inform policies aimed at reducing unjust inequalities in the justice system, both in the Czech Republic and internationally. ' },
                  ...
                  { id: 'WP4', title: 'Finance, Innovation, and Inequality', leader: 'Eva Horváthová', desc: 'Over the past three decades, economic inequality has risen significantly across many developed countries. This work package investigates the role of the financial sector, financial innovation, and financial crises in driving these trends, while also exploring whether inequality limits investment in new green technologies. At the individual level, the research looks at how disparities in financial and environmental literacy shape decision-making, drawing on innovative statistical methods and new data sources.' },
                  { id: 'WP5', title: 'Legal Aspects of Vulnerability', leader: 'Veronika Bílková', desc: 'New technologies serve both as a subject of research and as a tool for advancing methodology in this work package, which focuses on the legal dimensions of inequality and vulnerability. The research addresses three forms of vulnerability -- defined by personal status (such as age, ethnicity, or health), by situation (such as victims of international crime), and by argumentative position in legal proceedings. The findings will result in international publications as well as practical proposals for legislative reform.' },
      ...
      case 'intranet':
          ...
          return (
            <div className="py-24 px-6 max-w-md mx-auto text-center">
              <div className="bg-slate-50 p-10 rounded-2xl border border-slate-200">
                ...
                <h2 className="text-2xl font-bold mb-2" style={{ color: colors.navy }}>Restricted Access</h2>
        ...
        return (
          <div className="py-12 px-6 sm:px-12 max-w-6xl mx-auto animate-in fade-in duration-500">
  ...
  return (
    <div className="min-h-screen flex flex-col font-montserrat bg-white" style={{ color: colors.navy }}>
      ...
      <nav className="sticky top-0 z-50 bg-white shadow-sm h-32" onMouseLeave={() => setHoverTab(null)}>
